...
specify("db_replicate:setup", timeout, function (assert) {
    async.series(
        [ function (cb) {
            nano.db.create("db_replicate", cb);
        }
            , function (cb) {
            nano.db.create("db_replica", cb);
        }
            , function (cb) {
            nano.db.create("db_replica2", cb);
        }
        ]
...
specify("db_replicate:teardown", timeout, function (assert) {
    async.series(
        [ function (cb) {
            nano.db.destroy("db_replicate", cb);
        }
            , function (cb) {
            nano.db.destroy("db_replica", cb);
        }
            , function (cb) {
            nano.db.destroy("db_replica2", cb);
        }
        ]
        , function (error, results) {
