...
test('Error conditions', function (t) {
    ...
    function write(data, encoding) {
        ...
        return function () {
            feed.write(data, encoding)
        }
    }

    function end(data, encoding) {
        return function () {
            feed.end(data, encoding)
        }
    }
})
...
test('Longpoll feed', function (t) {
    for (var i = 0; i < 2; i++) {
        ...
        function encode(data) {
            return (i == 0) ? data : new Buffer(data)
        }

        function write(data) {
            return function () {
                feed.write(encode(data))
            }
        }

        function end(data) {
            return function () {
                feed.end(encode(data))
            }
        }
...
test('Pausing and destroying a feed mid-stream', function (t) {
    ...
    function check(bulk_docs_count) {
        ...
        var req = request({'uri': uri})
        req.on('response', function (res) {
            feed_response(null, res, res.body)
        })
