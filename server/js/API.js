...
var sessions = settings.getDB("sessions");
...
API = function () {
	...
	/**
	 * Returns a session from sessions db
	 * @param userId user who saved the session
	 * @param fn callback
	 */
	this.loadSession = function (userId, fn) {
		//get requested item
		sessions.get(userId, {}, function (err, body) {
			if (!err) {
				fn(body);
			}
			else {
	...
	/**
	 * Inserts a session into sessions db
	 * @param session session json to insert
	 * @param fn callback
	 */
	this.saveSession = function (session, fn) {
		sessions.get(session._id, {}, function (err, body) {
			if (!err) {
				session._rev = body._rev;
